...
/**
 * Explode a reviewer's fail verdict into individually triageable findings.
 * Idempotent per turn: a re-finalized turn replaces its own rows rather than
...
export function getReviewFindings(taskId: string): ReviewFinding[] {
  const db = getDb();
  return db.prepare(
    'SELECT * FROM review_findings WHERE task_id = ? ORDER BY created_at, position'
  ).all(taskId) as ReviewFinding[];
}

export function getReviewFinding(findingId: string): ReviewFinding | undefined {
  const db = getDb();
  return db.prepare('SELECT * FROM review_findings WHERE id = ?').get(findingId) as ReviewFinding | undefined;
}
...
export function setReviewFindingState(
  ...
  const db = getDb();
  // Returning to 'open' clears the decision so the finding reads as untriaged
  // again; the note is only meaningful alongside a decision.
  const decidedAt = state === 'open' ? null : new Date().toISOString();
  db.prepare('UPDATE review_findings SET state = ?, note = ?, decided_at = ? WHERE id = ?')
    .run(state, state === 'open' ? null : (note?.trim() || null), decidedAt, findingId);
}
...
/**
 * User replies after the one that created the task, oldest first. The creation
 * prompt is itself stored as the first user message and is surfaced separately
 * (as `task.prompt`), so it is excluded here.
 *
 * Auto-generated bodies are filtered out: the "Apply suggested fixes" button and
 * the per-finding fix action post the reviewer's own issue text as a user reply,
 * and replaying those to the reviewer would just echo its prior verdict back at
 * it as if the user had independently asked for it.
 */
export function getUserReplies(taskId: string): string[] {
  const db = getDb();
  const rows = db.prepare(
    "SELECT content FROM messages WHERE task_id = ? AND role = 'user' ORDER BY created_at, rowid"
  ).all(taskId) as Array<{ content: string }>;
  return rows
    .slice(1)
    .map(r => r.content)
    .filter(c => c && !REVIEW_FIX_REPLY_PREFIXES.some(p => c.startsWith(p)));
}

/** Marker prefix for replies generated by the per-finding fix action. */
export const REVIEW_FIX_REPLY_PREFIX = 'Please apply fixes for the issues the reviewer found';

/**
 * Every auto-generated "go fix the review" reply shape. The second is the
 * legacy "Apply suggested fixes" fallback used when a verdict carried no
 * issues list. Neither is real user direction, so neither is replayed.
 */
const REVIEW_FIX_REPLY_PREFIXES = [
